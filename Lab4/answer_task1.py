...
def skinning(joint_translation, joint_orientation, T_pose_joint_translation, T_pose_vertex_translation, skinning_idx, skinning_weight):
    ...
    for j in range(4):
        idx = skinning_idx[:,j]
        r = T_pose_vertex_translation - T_pose_joint_translation[idx]
        weight = np.expand_dims(skinning_weight[:,j], -1)
        trans = R.from_quat(joint_orientation[idx]).apply(r)+joint_translation[idx]
        vertex_translation += weight * trans
