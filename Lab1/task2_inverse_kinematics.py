...
def part1_simple(viewer, target_pos):
    ...
    meta_data = MetaData(joint_name, joint_parent, joint_initial_position, 'Hips', 'RightFoot')
...
def part1_hard(viewer, target_pos):
    ...
    viewer.create_marker(target_pos, [1, 0, 0, 1])
    joint_name, joint_parent, joint_initial_position = viewer.get_meta_data()
    meta_data = MetaData(joint_name, joint_parent, joint_initial_position, 'RightFoot', 'LeftFoot')
    joint_position = viewer.get_joint_positions()
    joint_orientation = viewer.get_joint_orientations()
...
def part2(viewer, bvh_name):
    ...
    right_foot_idx = joint_name.index('RightFoot')
    joint_position_fk, _ = part2_forward_kinematics(
                joint_name, joint_parent, joint_initial_position, motion_data, 0)
    origin_marker_pos = joint_position_fk[right_foot_idx]
    marker = viewer.create_marker_base(origin_marker_pos, [1, 0, 0, 1])
    meta_data = MetaData(bvh_joint_name, bvh_joint_parent, joint_initial_position[idx], 'RightUpLeg', 'RightFoot')

    class UpdateHandle:
        def __init__(self, marker, meta_data, motion_data, joint_offset):
            ...
            self.marker = marker
            self.trajectory = np.load('marker_trajectory.npy', allow_pickle=True).item()['marker_trajectory']
            
        def update_func(self, viewer):
            joint_position_fk, joint_orientation_fk = part2_forward_kinematics(
                self.joint_name, self.joint_parent, self.joint_offset, self.motion_data, self.current_frame)
            joint_position, joint_orientation = part2_inverse_kinematics(self.meta_data, joint_position_fk, joint_orientation_fk, 0.8)
            viewer.show_pose(self.joint_name, joint_position, joint_orientation)
            viewer.move_marker_to_pos(self.marker, self.trajectory[self.current_frame])
            self.current_frame = (self.current_frame + 1) % self.motion_data.shape[0]
            
            
    
    handle = UpdateHandle(marker, meta_data, motion_data, bvh_offset)
...
def main():
    ...
    # part1_hard(viewer, np.array([0.2, 0.5, -0.2]))
    # part1_animation(viewer, np.array([0.2, 0.5, -0.2]))
    
    # part2
    part2(viewer, 'data/spinkick.bvh')
